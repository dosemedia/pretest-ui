...
const TestObjective = observer(({ project, onSave }: { project: Project, onSave: (payload: object) => void }) => {
  ...
  function selectionCard (item: Branding) {
    return (
      <div className="flex flex-col" style={{ width: 285 }}>
        <div key={item.label} className={`card cursor-pointer ${branding === item.value && 'card-selected'}`} style={{ backgroundColor: 'white', padding: '0px 10px' }} onClick={() => setBranding(item.value)}>

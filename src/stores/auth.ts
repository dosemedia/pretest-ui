...
export class Auth {
  ...
  superadmin = localStorage.getItem('auth.superadmin') ? Boolean(localStorage.getItem('auth.superadmin')) : false
  ...
  constructor() {
    ...
    observe(this, 'superadmin', () => {
      localStorage.setItem('auth.superadmin', this.superadmin.toString())
    })
  ...
  async isSuperadmin(): Promise<void> {
    ...
    const isSuperadmin = Boolean(result.data?.isSuperadmin)
